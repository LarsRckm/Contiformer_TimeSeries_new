...
class ContiFormer(nn.Module):
    ...
    def calculate_loss(self, pred_x, target_x, cfg, time_interval,
                       weight_l1, weight_grad, weight_smooth, log, epoch, batch_idx, total):
        """
        ...
        log.info(f"Training: Iter: {epoch}, batch: {batch_idx/total}, L1 Loss: {l1_loss.item():.4f}")
        ...
        if time_interval is not None and len(time_interval) > 1:
            ...
            log.info(f"Training: Iter: {epoch}, batch: {batch_idx/total}, Gradient Loss: {gradient_loss.item():.4f}")
        ...
        if pred_x.shape[1] > 2:
            ...
            log.info(f"Training: Iter: {epoch}, batch: {batch_idx/total}, Smoothness Loss: {smooth_loss.item():.4f}")
        else:
